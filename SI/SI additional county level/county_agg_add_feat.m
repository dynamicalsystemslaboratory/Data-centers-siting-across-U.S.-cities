...
%% RETIRED COAL
T_rc = readtable("County_Total_Retired_Generators.csv", 'VariableNamingRule','preserve');
FIPS_rc = pad(string(T_rc{:,1}), 5, 'left','0');
val_rc  = force_numeric(T_rc{:,2});
val_rc = fillmissing(val_rc, 'constant', 0);
...
%% LAND PRICES (MSA-level)
% msa_land = readtable("cbsa_land_prices.csv", 'VariableNamingRule','preserve');
% msa_land = renamevars(msa_land, "MSA", "MSA");
% msa_land = msa_land(:, {'MSA','landval_mean_log'});
% msa_land = renamevars(msa_land, "landval_mean_log", "Value");
% msa_land.MSA = "C" + string(msa_land.MSA);
%% LAND PRICES (MSA-level via county crosswalk)
T_land = readtable("cbsa_land_prices.csv", 'VariableNamingRule','preserve');
T_land.MSA = string(T_land.MSA);
T_land.MSA = regexprep(T_land.MSA, '0$', '');
T_land.MSA = "C" + T_land.MSA;
msa_land = T_land(:, {'MSA','landval_mean_log'});
%% POVERTY (MSA-level)
% msa_pov = readtable("EP_POV100_cbsa.csv", 'VariableNamingRule','preserve');
% msa_pov = renamevars(msa_pov, "CBSA", "MSA");
% msa_pov = renamevars(msa_pov, "EP_POV100", "Value");
% msa_pov.MSA = "C" + string(msa_pov.MSA);
%% POVERTY (MSA-level via county crosswalk)
%% POVERTY (MSA-level)
T_pov = readtable("EP_POV100_cbsa.csv", 'VariableNamingRule','preserve');
T_pov.CBSA = string(T_pov.CBSA);
T_pov.MSA = regexprep(T_pov.CBSA, '0$', '');
T_pov.MSA = "C" + T_pov.MSA;
msa_pov = T_pov(:, {'MSA','EP_POV100'});

msa_land.MSA(msa_land.MSA == "C25775") = "C257A";
msa_land.MSA(msa_land.MSA == "C36837") = "C368A";

msa_pov.MSA(msa_pov.MSA == "C25775") = "C257A";
msa_pov.MSA(msa_pov.MSA == "C36837") = "C368A";
%msa_land = county_to_msa_by_fips( ...
 %   "county_land_prices.csv", 2, 1, crosswalk, 'mean');
% msa_pov = county_to_msa_by_fips( ...
%     "EP_POV100_county.csv", 2, 1, crosswalk, 'mean');
%% building all table
...
msa_all = add_var(msa_all, msa_land,           'landval_mean_log');
msa_all = add_var(msa_all, msa_pov,            'pov100');
...
writetable(msa_all, 'msa_features_all_def_eia_noprox_ret_review4.csv');
%% LAND PRICES (county-level)
T_land = readtable("county_land_prices.csv", 'VariableNamingRule','preserve');

FIPS_land = pad(string(T_land.CID), 5, 'left', '0');
val_land  = force_numeric(T_land.landval_mean_log);
county_all = add_var_county(county_all, FIPS_land, val_land, 'landval_mean_log');
%% POVERTY (county-level)
T_pov = readtable("EP_POV100_county.csv", 'VariableNamingRule','preserve');

FIPS_pov = pad(string(T_pov{:,1}), 5, 'left', '0');   % first column = FIPS
val_pov  = force_numeric(T_pov{:,2});                % second column = value

county_all = add_var_county(county_all, FIPS_pov, val_pov, 'pov100');
% T = readtable("your_file.csv");
% FIPS = pad(string(T{:,fips_col}), 5, 'left','0');
% VAL  = force_numeric(T{:,val_col});
% county_all = add_var_county(county_all, FIPS, VAL, 'your_var_name');
%%  COUNTY-LEVEL 
county_all.FIPS = string(county_all.FIPS);
county_all = sortrows(county_all, 'FIPS');
% remove county_all.nri_events, county_all.area
county_all = county_all(:,[1 3:5 8:end]);
writetable(county_all, 'county_features_all_nh_area_eia_noprox_ret_review3.csv');
